...
class Part:
    """A single part of a multipart body (headers + body stream).

    Provides write_to() for streaming and bytes_left_to_write() for progress.
    peek() was considered per Issue #624 but not implemented: the encoder
    design streams via write_to() without requiring look-ahead.
    """
    ...
    def rewind(self) -> None:
        ...
        :raises UnrewindableBodyError: If the body stream has no seek() method
            or its seekable() method returns False.
        """
        self.headers_unread = True
        body = self.body
    ...
    def bytes_left_to_write(self) -> int:
        ...
        :returns: int -- bytes remaining to write
        """
        to_read = 0
        if self.headers_unread:
            to_read += len(self.headers)

        return to_read + total_len(self.body)
...
Fields = typing.Union[
    ...
    typing.Sequence[tuple[str, FieldValue]],
]
    ...
    def _iter_fields(self) -> typing.Iterator[RequestField]:
        for name, v in to_list(self._fields):
            ...
            if isinstance(v, (list, tuple)):
                ...
                elif filename is None and hasattr(data, "read"):
                    # File-like without .name (e.g. BytesIO) - use fallback per
                    # requests-toolbelt PR-316 for consistent Content-Disposition
                    filename = "unknown"
...
def coerce_data(
    ...
    if not isinstance(data, _CustomBytesIO):
        ...
        # Fallback for BinaryIO-like objects that have read() but no fileno()
        # (e.g. custom in-memory streams).  Read the entire content into a
        # _CustomBytesIO so that length calculation and buffering work correctly.
        if hasattr(data, "read"):
            ...
            return _CustomBytesIO(data.read(), encoding)
