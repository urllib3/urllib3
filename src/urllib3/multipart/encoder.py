...
class Part:
    """A buffered multipart body with serialized headers.

    Create a part from a :class:`~urllib3.fields.RequestField` using
    :meth:`from_field`. ``read()`` and ``peek()`` expose body bytes, excluding
    the serialized headers. ``seek(0)`` restores the original body position.
    The caller owns the input stream and must keep it open and unchanged.
    """
    ...
    def __init__(
        ...
        self._peeked = b""
        self._remaining = self.len - len(self.headers)

    @classmethod
    def from_field(cls, field: fields.RequestField, encoding: str = "utf-8") -> Part:
        """Create a part from a Request Field generated by urllib3."""
        headers = to_bytes(field.render_headers(), encoding)
    ...
    def bytes_left_to_write(self) -> bool:
        ...
        return (to_read + len(self._peeked) + self._remaining) > 0
    ...
    def write_to(self, buffer: _CustomBytesIO, size: int) -> int:
        ...
        while (self._peeked or self._remaining > 0) and (size == -1 or written < size):
            amount_to_read = size
            if size != -1:
                amount_to_read = size - written
            written += buffer.append(self.read(amount_to_read))

        return written

    def read(self, size: int = -1) -> bytes:
        """Read body bytes, including any bytes buffered by :meth:`peek`."""
        if size == 0:
            return b""
        buffered = self._peeked if size < 0 else self._peeked[:size]
        self._peeked = self._peeked[len(buffered) :]
        remaining = self._remaining
        amount = remaining if size < 0 else min(remaining, size - len(buffered))
        chunks = [buffered]
        while amount > 0:
            chunk = self.body.read(min(amount, 65536))
            if not chunk:
                raise OSError("Unexpected end of file while encoding multipart body")
            chunks.append(chunk)
            amount -= len(chunk)
    ...
    def peek(self, size: int = 0) -> bytes:
        """Return buffered body bytes without advancing the logical position."""
        if not self._peeked:
            self._peeked = self.read(max(1, min(size, 65536)))
        return self._peeked

    def seek(self, offset: int, whence: int = 0) -> int:
        """Rewind to the body position captured when this part was created."""
        if offset != 0 or whence != 0:
            raise io.UnsupportedOperation("Multipart parts only support seek(0, 0)")
...
class _CustomBytesIO(io.BytesIO):
    def __init__(
        ...
        if buffer is None:
            buffer = b""
        bufferbytes = to_bytes(buffer, encoding)
...
class FileWrapper:
    ...
    @property
    def len(self) -> int:
        return max(0, self._total_len - self.fd.tell())

    def read(self, length: int = -1) -> bytes:
        remaining = self.len
        if length < 0:
            length = remaining
        return self.fd.read(min(length, remaining))
...
class MultipartEncoder:
    ...
    dict or more simply, a list of tuples:
    ...
    def __init__(
        ...
        *,
        ...
        self._position = 0
        self._rewind_error: OSError | None = None
        ...
        self._encoded_boundary: bytes = (self._boundary + "\r\n").encode("latin-1")
        self._fields = fields
        self._iter_read_size = blocksize
    ...
    def __iter__(self) -> typing.Iterator[bytes]:
        while chunk := self.read(self._iter_read_size):
            yield chunk

    def __next__(self) -> bytes:
        chunk = self.read(self._iter_read_size)
        if not chunk:
            raise StopIteration
        return chunk
    ...
    def _calculate_length(self) -> int:
        ...
        """
        self._len = (
            sum(len(self._encoded_boundary) + part.len + 2 for part in self._parts)
            + len(self._encoded_boundary)
            + 2
        )
        return self._len
    ...
    def _iter_fields(self) -> typing.Iterator[RequestField]:
        for item in to_list(self._fields):
            ...
            field = fields.RequestField(
                name=name,
                # RequestField only renders metadata here; Part handles the stream.
                data=typing.cast(typing.Any, data),
                filename=filename,
                headers=headers,
            )
    ...
    def tell(self) -> int:
        """Return the number of encoded bytes consumed."""
        return self._position

    def seek(self, offset: int, whence: int = 0) -> int:
        """Rewind all parts to their original positions for request retries.

        Only ``seek(0, 0)`` is supported. Input streams must remain open and
        unchanged. If any part cannot be rewound, an ``OSError`` is raised.
        """
        if offset != 0 or whence != 0:
        ...
        for part in self._parts:
            try:
                ...
                error = OSError("Unable to rewind multipart body part")
                error.__cause__ = exc
        self._rewind_error = error
        ...
        self._buffer = _CustomBytesIO(encoding=self._enc)
        self._write_boundary()
        if not self._parts:
            self._write_closing_boundary()
            self._finished = True
