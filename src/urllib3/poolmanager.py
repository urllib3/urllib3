...
from ._collections import HTTPHeaderDict, HTTPHeaderMapping, RecentlyUsedContainer
...
def _default_key_normalizer(
    ...
    for key in ("headers", "_proxy_headers", "_socks_options"):
        if key in context and context[key] is not None:
            if key == "_socks_options":
                context[key] = frozenset(context[key].items())
            else:
                context[key] = frozenset(HTTPHeaderDict(context[key]).items())
...
class ProxyManager(PoolManager):
    ...
    def __init__(
        ...
        num_pools: int = 10,
        headers: HTTPHeaderMapping | None = None,
        proxy_headers: HTTPHeaderMapping | None = None,
    ...
    def _set_proxy_headers(
        self, url: str, headers: HTTPHeaderMapping | None = None
    ) -> HTTPHeaderDict:
        """
