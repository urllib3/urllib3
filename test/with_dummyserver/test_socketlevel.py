    ...
    def test_recovery_when_server_closes_connection(self):
        ...
        def socket_handler(listener):
            for i in 0, 1, 2, 3, 4:
                ...
                # only interact with client on every 2nd request. this way we get
                # to test the retry mechanism as well
                if i % 2 != 0:
                    buf = b''
                    while not buf.endswith(b'\r\n\r\n'):
                        buf = sock.recv(65536)
        ...
        response = pool.request('GET', '/', retries=1)
        self.assertEqual(response.status, 200)
        self.assertEqual(response.data, b'Response 3')

        done_closing.wait()  # wait until the socket in our pool gets closed

        # without retries, we should fail with exception
