...
class TestRetry:
    ...
    def test_backoff_first_retry(self) -> None:
        """Backoff from first retry is computed correctly"""
        max_backoff = Retry.DEFAULT_BACKOFF_MAX

        retry = Retry(total=100, backoff_factor=0.2, backoff_first_retry=True)
        assert retry.get_backoff_time() == 0  # First request

        retry = retry.increment(method="GET")
        assert retry.get_backoff_time() == 0.2  # First retry: start backoff
