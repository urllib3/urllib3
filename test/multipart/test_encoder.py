...
class TestMultipartEncoder(unittest.TestCase):
    ...
    def test_memory_usage_streaming_large_file(self) -> None:
        """Verify MultipartEncoder streams file content without loading it fully.

        When streaming a 10 MB file from disk, peak memory should stay well under
        3 MB (chunked reads only). If the encoder buffered the entire file like
        encode_multipart_formdata, we would exceed this limit (10 MB source +
        10 MB buffer). This test satisfies Issue #624's memray verification.
        """
        file_size_mb = 10
        chunk_size = 8192 * 4  # default_iter_read_size
    ...
    def test_encodes_with_readable_data(self) -> None:
        ...
        assert m.read() == (
            ...
            b'Content-Disposition: form-data; name="field"; filename="unknown"\r\n\r\n'
            b"value\r\n"
            b"--this-is-a-boundary--\r\n"
        )
    ...
    def test_filename_fallback_for_file_like_without_name(self) -> None:
        """When a file-like without .name (e.g. BytesIO) is passed as a bare
        value, use 'unknown' as filename fallback per requests-toolbelt PR #316."""
        m = MultipartEncoder([("file", io.BytesIO(b"content"))])
        body = m.read().decode("utf-8", errors="replace")
        assert 'filename="unknown"' in body
